...
required_performance_streak = 80
...
def setup(num_in, num_out):
    ...
    input_num = num_in
    output_num = num_out
    target_dir = os.path.join(os.getcwd(),'valid_models')#'C:\Users\NpEri\Desktop\4th_year\Capstone\3_20_2023\eamon\MobiCharged\src\valid_models'
    ...
    json_path = os.path.join(os.getcwd(), 'valid_models', 'startup_data.json')
...
def save_best_weights():
    ...
    current_best[1].model.save(os.path.join(os.getcwd(),'current_best','model')) #ADDED THIS
    current_best[1].get_model().save_weights(save_path)

def is_best_present():
    return os.path.exists(os.path.join(os.getcwd(), 'current_best', 'best_weights.h5'))

def get_current_best_path():
    return os.path.join(os.getcwd(), 'current_best', 'best_weights.h5')
...
def get_best_model_name():
    source_path = os.path.join(os.getcwd(), 'current_best', 'best.txt')
