...
class MyGUI:
    def __init__(self, master):
        ...
        # #THREADING TESTING REMOVE -------------------------------------------------------
        # self.label = tk.Label(master, text="Values:")
        # self.label.pack()
        # self.value = None

        # self.predict_label = tk.Label(master,text = "Prediction Output = ")
        # self.predict_label.pack()
        # #REMOVE -------------------------------------------------------


        # start a timer to update the label and image periodically
        self.update_image()
        #self.update_label()
    ...
    def call_tfpredict(self):
        ...
        if model_name != None:
            ...
            if (output != None): #just in case model folder gets deleted, but best.txt exists.
                ...
                #self.predict_label.config(text="Prediction Output = " + str(output))
                messagebox.showinfo("Success", "Inputs saved successfully!")
                return model_name ,output
            else:
                messagebox.showerror("Error 0","Error, there is no file named 'best.txt', Please press start first.")
...
### TESTING NEW CONCURRENCY WITH QUEUE, NEED TO IMPLEMENT ML BLACKBOARD TRAINING
    # def update_label(self):
    #     try:
    #         item = queue_module.my_queue.get(timeout=0.1)
    #         print('Update_label')
    #         if item is not None:
    #             #item = queue_module.my_queue.get()
    #             print(str(item))
    #             # get the current values and update the label
    #             values = item
    #             #value_str = ", ".join(str(v) for v in values)
    #             value_str = str(values)
    #             self.label.config(text="Values: " + value_str)
    #     except Exception:
    #         pass
    #     #update the label every 1000 ms
...
def main():
    ...
    #mlb_thread = threading.Thread(target = calculation.calculate)
    #mlb_thread.start()
    #mlb_thread.join()
    root.mainloop()
    #MyGUI.train_thread(gui)
